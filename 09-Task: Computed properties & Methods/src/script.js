Vue.createApp({
  ...
  computed: {
    tableColumnHeadNames() {
      ...
      const oneBookItem = this.books[0];

      const columnResult = Object.keys(oneBookItem).filter((oneKey) => {
        return validKeys.includes(oneKey);
      });
  ...
  methods: {
    calculateDiscount(dollar, discount) {
